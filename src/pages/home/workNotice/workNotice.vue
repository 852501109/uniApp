<template>
	<view class="workNotice">
		...
		<view class="top-warp">
			<me-tabs  v-model="tabIndex" :tabs="tabList" @change="tabChange" :tab-width="180"></me-tabs>
		...
		<view slot="content">
			...
			<view class="searchContent">
				...
				<view class="yqTime">
					<view class="yqItem" :key="idnex" @tap="post.day=item.value" :class="item.value == post.day?'active':''" v-for="(item,idnex) in yqTime">{{item.text}}</view>
				</view>
...
<script>
	...
	import { updatePlan,getWorkTag } from '@/request/notice.js'
	...
	export default {
		components: { Metabs,NoticeItem,selfSearchPop,selfSearchInput,myPicker,MxDatePicker,selfRadio,selfSignPop },
		mixins: [MescrollMoreMixin], // 使用mixin
		data() {
			return {
				tabList:[
					{ name: '高血压',num:0,type:'GXY'}, { name: '糖尿病',num:0,type:'TNB'},
		...
		created() {
			...
			config.existJsb == '是'?this.tabList.push({"name": "严重精神障碍","type": "JSB"}):this.delArr('JSB',this.tabList)
			config.existYcf == '是'?this.tabList.push({"name": "孕产妇","type": "YCF"}):this.delArr('YCF',this.tabList)
			config.existEt == '是'?this.tabList.push({"name": "0-6儿童","type": "ET"}):this.delArr('ET',this.tabList)	
		},
		methods:{
			//总数赋值
			getNum(num){
				console.log('触发赋值',num)
				this.totalNum = num || 0
			},
			// 根据type删除配置
			delArr(type,arr){
			...
			searchHandle(){
				...
				this.$refs.mescrollItem[this.tabIndex].initList(this.post)
			},
			yqHandle(){
			...
			// 确定延迟
			okLaterHandle(){
				this.late.type = this.tabList[this.tabIndex].type
